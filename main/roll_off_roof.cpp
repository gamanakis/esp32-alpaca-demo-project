...
RollOffRoof::RollOffRoof() : Dome() { _connected = false; }

RollOffRoof::~RollOffRoof() {}

esp_err_t RollOffRoof::action(const char *action, const char *parameters,
                              char *buf, size_t len) {
...
esp_err_t RollOffRoof::commandstring(const char *action, bool raw, char *buf,
                                     size_t len) {
  return ALPACA_ERR_ACTION_NOT_IMPLEMENTED;
}
...
esp_err_t RollOffRoof::put_park() { return ALPACA_ERR_NOT_IMPLEMENTED; }
